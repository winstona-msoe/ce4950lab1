...
void CLOCK2_Start(void) 
{
    ...
    CLOCK2_CLKEN |= CLOCK2_CLKEN_MASK;
	CLOCK2_CLKSTBY |= CLOCK2_CLKSTBY_MASK;
}
...
void CLOCK2_Stop(void) 
{
    ...
    CLOCK2_CLKEN &= (uint8)(~CLOCK2_CLKEN_MASK);
	CLOCK2_CLKSTBY &= (uint8)(~CLOCK2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLOCK2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLOCK2_CLKEN & CLOCK2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLOCK2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLOCK2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(CLOCK2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLOCK2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLOCK2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLOCK2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLOCK2_GetSourceRegister();
    uint16 oldDivider = CLOCK2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLOCK2_CLKEN & CLOCK2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(CLOCK2_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLOCK2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK2_MOD_SRC |= modeBitMask & (uint8)CLOCK2_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK2_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLOCK2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK2_MOD_SRC & (uint8)(CLOCK2_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLOCK2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLOCK2_GetDividerRegister();
    uint8 oldSrc = CLOCK2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK2_MOD_SRC |= CYCLK_SSS;
        CLOCK2_MOD_SRC =
            (CLOCK2_MOD_SRC & (uint8)(~CLOCK2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLOCK2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK2_MOD_SRC & CLOCK2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLOCK2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK2_PHASE = clkPhase & CLOCK2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLOCK2_PHASE & CLOCK2_PHASE_MASK;
}
