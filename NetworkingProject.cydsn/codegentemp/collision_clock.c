...
/*******************************************************************************
* Function Name: collision_clock_Start
********************************************************************************
*
* Summary:
...
void collision_clock_Start(void) 
{
    ...
    collision_clock_CLKEN |= collision_clock_CLKEN_MASK;
	collision_clock_CLKSTBY |= collision_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: collision_clock_Stop
********************************************************************************
*
* Summary:
...
void collision_clock_Stop(void) 
{
    ...
    collision_clock_CLKEN &= (uint8)(~collision_clock_CLKEN_MASK);
	collision_clock_CLKSTBY &= (uint8)(~collision_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: collision_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((collision_clock_CLKEN & collision_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = collision_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = collision_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* collision_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(collision_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(collision_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: collision_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: collision_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = collision_clock_GetSourceRegister();
    uint16 oldDivider = collision_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = collision_clock_CLKEN & collision_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(collision_clock_DIV_PTR, clkDivider);
                collision_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                collision_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(collision_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((collision_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: collision_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(collision_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: collision_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    collision_clock_MOD_SRC |= modeBitMask & (uint8)collision_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: collision_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    collision_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(collision_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: collision_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return collision_clock_MOD_SRC & (uint8)(collision_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: collision_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = collision_clock_GetDividerRegister();
    uint8 oldSrc = collision_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        collision_clock_MOD_SRC |= CYCLK_SSS;
        collision_clock_MOD_SRC =
            (collision_clock_MOD_SRC & (uint8)(~collision_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: collision_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return collision_clock_MOD_SRC & collision_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: collision_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    collision_clock_PHASE = clkPhase & collision_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: collision_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return collision_clock_PHASE & collision_clock_PHASE_MASK;
}
