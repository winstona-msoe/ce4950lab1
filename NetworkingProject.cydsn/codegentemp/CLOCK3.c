...
void CLOCK3_Start(void) 
{
    ...
    CLOCK3_CLKEN |= CLOCK3_CLKEN_MASK;
	CLOCK3_CLKSTBY |= CLOCK3_CLKSTBY_MASK;
}
...
void CLOCK3_Stop(void) 
{
    ...
    CLOCK3_CLKEN &= (uint8)(~CLOCK3_CLKEN_MASK);
	CLOCK3_CLKSTBY &= (uint8)(~CLOCK3_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLOCK3_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLOCK3_CLKEN & CLOCK3_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLOCK3_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLOCK3_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(CLOCK3_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLOCK3_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLOCK3_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLOCK3_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLOCK3_GetSourceRegister();
    uint16 oldDivider = CLOCK3_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLOCK3_CLKEN & CLOCK3_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK3_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(CLOCK3_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLOCK3_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK3_MOD_SRC |= modeBitMask & (uint8)CLOCK3_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK3_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK3_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK3_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLOCK3_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK3_MOD_SRC & (uint8)(CLOCK3_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLOCK3_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLOCK3_GetDividerRegister();
    uint8 oldSrc = CLOCK3_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK3_MOD_SRC |= CYCLK_SSS;
        CLOCK3_MOD_SRC =
            (CLOCK3_MOD_SRC & (uint8)(~CLOCK3_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLOCK3_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK3_MOD_SRC & CLOCK3_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLOCK3_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK3_PHASE = clkPhase & CLOCK3_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK3_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLOCK3_PHASE & CLOCK3_PHASE_MASK;
}
