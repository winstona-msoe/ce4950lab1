...
/*******************************************************************************
* Function Name: CollisionDelayClk_Start
********************************************************************************
*
* Summary:
...
void CollisionDelayClk_Start(void) 
{
    ...
    CollisionDelayClk_CLKEN |= CollisionDelayClk_CLKEN_MASK;
	CollisionDelayClk_CLKSTBY |= CollisionDelayClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CollisionDelayClk_Stop
********************************************************************************
*
* Summary:
...
void CollisionDelayClk_Stop(void) 
{
    ...
    CollisionDelayClk_CLKEN &= (uint8)(~CollisionDelayClk_CLKEN_MASK);
	CollisionDelayClk_CLKSTBY &= (uint8)(~CollisionDelayClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CollisionDelayClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CollisionDelayClk_CLKEN & CollisionDelayClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CollisionDelayClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CollisionDelayClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CollisionDelayClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CollisionDelayClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CollisionDelayClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CollisionDelayClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CollisionDelayClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CollisionDelayClk_GetSourceRegister();
    uint16 oldDivider = CollisionDelayClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CollisionDelayClk_CLKEN & CollisionDelayClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CollisionDelayClk_DIV_PTR, clkDivider);
                CollisionDelayClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CollisionDelayClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CollisionDelayClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CollisionDelayClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CollisionDelayClk_DIV_PTR, clkDivider);
				CollisionDelayClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CollisionDelayClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CollisionDelayClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: CollisionDelayClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CollisionDelayClk_MOD_SRC |= modeBitMask & (uint8)CollisionDelayClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: CollisionDelayClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CollisionDelayClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CollisionDelayClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: CollisionDelayClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CollisionDelayClk_MOD_SRC & (uint8)(CollisionDelayClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: CollisionDelayClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CollisionDelayClk_GetDividerRegister();
    uint8 oldSrc = CollisionDelayClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CollisionDelayClk_MOD_SRC |= CYCLK_SSS;
        CollisionDelayClk_MOD_SRC =
            (CollisionDelayClk_MOD_SRC & (uint8)(~CollisionDelayClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CollisionDelayClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CollisionDelayClk_MOD_SRC & CollisionDelayClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CollisionDelayClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CollisionDelayClk_PHASE = clkPhase & CollisionDelayClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CollisionDelayClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CollisionDelayClk_PHASE & CollisionDelayClk_PHASE_MASK;
}
